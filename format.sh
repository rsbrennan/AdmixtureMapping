...
#make target header
zcat ~/admixture_mapping/variants/all.filtered.vcf.gz | head -n 400 | grep '^#' | head -n 57 | tail -n 1 | \
...
zcat ~/admixture_mapping/variants/all.filtered.vcf.gz | head -n 400 | grep '^#' | head -n 56 | \
