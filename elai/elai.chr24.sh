...
cat chr24.HP.map | awk -F'\t' -vOFS='\t' '{ gsub("24", "x24", $1) ; print }' > chr24.HP.1.map
cat chr24.BC.map | awk -F'\t' -vOFS='\t' '{ gsub("24", "x24", $1) ; print }' > chr24.BC.1.map
cat chr24.PC.map | awk -F'\t' -vOFS='\t' '{ gsub("24", "x24", $1) ; print }' > chr24.PC.1.map

mv chr24.AF.1.map chr24.AF.map
mv chr24.PP.1.map chr24.PP.map
mv chr24.PL.1.map chr24.PL.map
mv chr24.HP.1.map chr24.HP.map
mv chr24.BC.1.map chr24.BC.map
mv chr24.PC.1.map chr24.PC.map
