...
### remove outliers and calulcate sliding average
...
df<- matrix(ncol=3)
for(i in 1:24){
	chr <- which(pp.pl$CHR==i)
	for(snp in 1:(length(chr)-1){
		a <- c(1)
		dist <- pp.pl$BP[chr[snp]] - pp.pl$BP
		if(abs(dist[snp]) < 100000){
			a<- #want to bind these together with avg position of the group.

		}
	}
}

pp.pl.win <- read.table("PP_vs_PL.window.windowed.weir.fst", header=TRUE)
pp.pl.win$fst <- pp.pl.win$WEIGHTED_FST
pp.pl.win$fst[which(pp.pl.win$fst < 0)] <- c(0)
pp.pl.win$fst[which(pp.pl.win$N_VARIANTS == 1)]	<- c(NA)
